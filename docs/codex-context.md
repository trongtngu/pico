...
- Auth/session:
  ...
  - Email signup now supports a Turnstile CAPTCHA token supplied by `pico/BotChallengeView.swift`; the challenge is enabled only when `PICO_TURNSTILE_SITE_KEY` resolves from the app Info.plist/build settings. Supabase CAPTCHA must also be enabled in the hosted auth settings with the matching secret.
- Onboarding:
