...
- `StorePage` loads `BerryStore` inventory and then calls `berryStore.applyOwnedStoreItemIDs(sessionStore.ownedStoreItemIDs)`, meaning the auth store’s ownership can override the just-fetched BerryStore ownership in that view. This may be intentional because `AuthSessionStore` is the profile/inventory authority, but the ownership source-of-truth is not fully clear (`pico/ContentView.swift:6836`, `pico/ContentView.swift:6841`).
- `AuthService.isUsernameAvailable` passes the anon key as the bearer token for a public profile query (`pico/AuthService.swift:347`). This may rely on public RLS and may be intentional; do not change without checking policies.
- `FriendService.searchProfiles` manually sanitizes query text and builds an `ilike` pattern in a URL query item (`pico/FriendService.swift:60`). It avoids some characters, but search semantics and escaping behavior should be considered before extending it.
