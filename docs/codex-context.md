...
- Pico Plus:
  ...
  - Paywall presentation is centralized through `PicoPlusStore.presentPaywall(source:authSession:)`; feature views should pass a `PicoPlusPaywallSource` with an explicit `PicoPlusPlacement` rather than calling Superwall placements directly. Paywalls require an authenticated Supabase session so Superwall purchases can be linked to the Supabase user ID. The shared CTA is `PicoPlusCTAButton` in `pico/PicoPlusPaywallView.swift`.
  - The onboarding flow does not present a pre-auth paywall. It sets a one-shot pending onboarding Plus paywall on `AuthSessionStore`, and `AppShellView` presents it only after profile loading/completion and Superwall identity configuration.
  - The Superwall webhook writes `user_plus_entitlements` only when the event contains an exact Supabase UUID from the identified app user/user attributes; anonymous `$SuperwallAlias` IDs are intentionally not used as entitlement owner IDs.
