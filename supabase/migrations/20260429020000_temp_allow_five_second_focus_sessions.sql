-- TODO: REVERT THIS - restore the 60-second focus duration minimum after short-session testing.
alter table public.focus_sessions
    drop constraint if exists focus_sessions_duration_seconds;

alter table public.focus_sessions
    add constraint focus_sessions_duration_seconds
        check (duration_seconds between 5 and 86400);
...
begin
    ...
    if duration_seconds is null or duration_seconds < 5 or duration_seconds > 86400 then
        raise exception 'Focus session duration must be between 5 seconds and 24 hours.' using errcode = '22023';
    end if;
