-- Complete profile cutover cleanup for databases that already ran the legacy
-- avatar/profile migrations. This intentionally drops app-owned profile rows;
-- auth.users remains the source of auth identity.
...
drop function if exists public.get_my_avatar_key();
drop function if exists public.update_my_avatar_key(text);
